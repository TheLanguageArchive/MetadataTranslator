...
<xsl:stylesheet xmlns="http://www.mpi.nl/IMDI/Schema/IMDI" 
    ...
    <xsl:template name="SILANGSESSION2IMDISESSION">
        <METATRANSCRIPT xmlns="http://www.mpi.nl/IMDI/Schema/IMDI"
            ...
            <xsl:attribute name="Originator">
                <xsl:value-of select="tla:create-originator('SILANGSESSION2IMDISESSION.xslt', //Header/MdSelfLink)" />
            </xsl:attribute>
    ...
    <xsl:template match="lat-SL-session" mode="SILANGSESSION2IMDISESSION">
        ...
        <Session>
            ...
            <xsl:apply-templates select="/CMD/Resources/ResourceProxyList" mode="SILANGSESSION2IMDISESSION" />
            <References />
        </Session>
    </xsl:template>
    ...
    <xsl:template match="Content" mode="SILANGSESSION2IMDISESSION">
        <Content>
            ...
            <xsl:if test="normalize-space(child::ElicitationMethod)!=''">
                <Key Name="ElicitationMethod"><xsl:value-of select="ElicitationMethod" /></Key>
            </xsl:if>
    ...
    <xsl:template match="SL_Interpreting" mode="SILANGSESSION2IMDISESSION">
        <xsl:if test="normalize-space(child::Source)!=''">
            <Key Name="Interpreting.Source"><xsl:value-of select="Source" /></Key>
        </xsl:if>
        <xsl:if test="normalize-space(child::Target)!=''">
            <Key Name="Interpreting.Target"><xsl:value-of select="Target" /></Key>
        </xsl:if>
        <xsl:if test="normalize-space(child::Visibility)!=''">
            <Key Name="Interpreting.Visibility"><xsl:value-of select="Visibility" /></Key>
        </xsl:if>
        <xsl:if test="normalize-space(child::Audience)!=''">
            <Key Name="Interpreting.Audience"><xsl:value-of select="Audience" /></Key>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="Actor" mode="SILANGSESSION2IMDISESSION">
        <Actor>
            ...
            <Keys>
                ...
                <xsl:apply-templates select="SL_SignLanguageExperience" mode="SILANGSESSION2IMDISESSION"/>
    ...
    <xsl:template match="SL_Deafness" mode="SILANGSESSION2IMDISESSION">
        <xsl:if test="normalize-space(child::Status)!=''">
            <Key Name="Deafness.Status"><xsl:value-of select="Status" /></Key>
        </xsl:if>
        <xsl:if test="normalize-space(child::AidType)!=''">
            <Key Name="Deafness.AidType"><xsl:value-of select="AidType" /></Key>
        </xsl:if>
    </xsl:template>
    
    <xsl:template match="SL_SignLanguageExperience" mode="SILANGSESSION2IMDISESSION">
        <xsl:if test="normalize-space(child::ExposureAge)!=''">
            <Key Name="SignLanguageExperience.ExposureAge"><xsl:value-of select="ExposureAge" /></Key>
        </xsl:if>
        <xsl:if test="normalize-space(child::AcquisitionLocation)!=''">
            <Key Name="SignLanguageExperience.AcquisitionLocation"><xsl:value-of select="AcquisitionLocation" /></Key>
        </xsl:if>
        <xsl:if test="normalize-space(child::SignTeaching)!=''">
            <Key Name="SignLanguageExperience.SignTeaching"><xsl:value-of select="SignTeaching" /></Key>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="SL_Mother|SL_Father|SL_Partner" mode="SILANGSESSION2IMDISESSION">
        <xsl:if test="normalize-space(child::Deafness)!=''">
            <Key Name="{concat('Family.',replace(name(.),'SL_',''),'.Deafness')}"><xsl:value-of select="Deafness" /></Key>
        </xsl:if>
        <xsl:if test="normalize-space(child::PrimaryCommunicationForm)!=''">
            <Key Name="{concat('Family.',replace(name(.),'SL_',''),'.PrimaryCommunicationForm')}"><xsl:value-of select="PrimaryCommunicationForm" /></Key>
        </xsl:if>
    </xsl:template>
    
    <xsl:template match="SL_Education" mode="SILANGSESSION2IMDISESSION">
        <xsl:if test="normalize-space(child::Age)!=''">
            <Key Name="Education.Age"><xsl:value-of select="Age" /></Key>
        </xsl:if>
        <xsl:if test="normalize-space(child::SchoolType)!=''">
            <Key Name="Education.SchoolType"><xsl:value-of select="SchoolType" /></Key>
        </xsl:if>
        <xsl:if test="normalize-space(child::ClassKind)!=''">
            <Key Name="Education.ClassKind"><xsl:value-of select="ClassKind" /></Key>
        </xsl:if>
        <xsl:if test="normalize-space(child::EducationModel)!=''">
            <Key Name="Education.EducationModel"><xsl:value-of select="EducationModel" /></Key>
        </xsl:if>
        <xsl:if test="normalize-space(child::Location)!=''">
            <Key Name="Education.Location"><xsl:value-of select="Location" /></Key>
        </xsl:if>
        <xsl:if test="normalize-space(child::BoardingSchool)!=''">
            <Key Name="Education.BoardingSchool"><xsl:value-of select="BoardingSchool" /></Key>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="ResourceProxy[ResourceType = 'LandingPage']" mode="SILANGSESSION2IMDISESSION-MEDIAFILE">
    </xsl:template>
    
    <xsl:template match="ResourceProxy[ResourceType = 'Resource']" mode="SILANGSESSION2IMDISESSION-MEDIAFILE">
    ...
    <xsl:template match="MediaFile" mode="SILANGSESSION2IMDISESSION">        
        <xsl:call-template name="sl-generate-ResourceId"></xsl:call-template>
        <MediaFile>
