...
<xsl:stylesheet xmlns="http://www.mpi.nl/IMDI/Schema/IMDI" 
    ...
    <xsl:template name="TALKBANK2IMDI">
        <METATRANSCRIPT xmlns="http://www.mpi.nl/IMDI/Schema/IMDI"
            ...
            <xsl:attribute name="Originator">
                <xsl:value-of select="tla:create-originator('TALKBANK2IMDI.xslt', //Header/MdSelfLink)" />
            </xsl:attribute>
            <xsl:attribute name="Date">
                <xsl:value-of select="//Header/MdCreationDate" />
            </xsl:attribute>
            <xsl:apply-templates select="//talkbank-session/Session" mode="TALKBANK2IMDI"/>
        </METATRANSCRIPT>
    </xsl:template>
    
    <xsl:template match="talkbank-session/Session" mode="TALKBANK2IMDI">
        <History>
            <xsl:if test="normalize-space(child::History)!=''">
                    <xsl:value-of select="child::History"/>
                    <xsl:text> </xsl:text>
            </xsl:if>                   
            <xsl:text> NAME:TALKBANK2IMDI.xslt DATE:</xsl:text>
