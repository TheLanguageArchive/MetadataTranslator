...
<xsl:stylesheet xmlns="http://www.mpi.nl/IMDI/Schema/IMDI"
    ...
    <xsl:template name="TALKBANKCOLLECTION2CORPUS">
        <METATRANSCRIPT xmlns="http://www.mpi.nl/IMDI/Schema/IMDI"
            ...
            <xsl:attribute name="Originator">
                <xsl:value-of
                    select="tla:create-originator('TALKBANKCOLLECTION2CORPUS.xslt', //Header/MdSelfLink)"/>
            </xsl:attribute>
            <Corpus>
                <xsl:apply-templates select="//collection" mode="TALKBANKCOLLECTION2CORPUS"/>
                <xsl:apply-templates select="//ResourceProxy" mode="TALKBANKCOLLECTION2CORPUS"/>
            </Corpus>
        </METATRANSCRIPT>
    </xsl:template>

    <xsl:template match="ResourceProxy" mode="TALKBANKCOLLECTION2CORPUS">
    ...
    <xsl:template match="collection" mode="TALKBANKCOLLECTION2CORPUS">
        <Name>
            <xsl:value-of select="CollectionInfo/Name"/>
        </Name>
        <Title>
            <xsl:value-of select="CollectionInfo/Title"/>
        </Title>
